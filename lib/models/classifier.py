...
class Interventional_Classifier(nn.Module):
    def __init__(self, num_classes=80, feat_dim=2048, num_head=4, tau=32.0, beta=0.03125, *args):
        ...
        self.head_dim = feat_dim // num_head # 512 in resnet101
...
if __name__ == '__main__':
    ...
    num_classes = 80
    feat_dim = 2048
    Classifier = context_clf = Interventional_Classifier(num_classes=num_classes, feat_dim=feat_dim, num_head=4, tau=32.0, beta=0.03125)
    Classifier.cuda().eval()
    print(Classifier)
    image = torch.randn(4, 2048)
    image = image.cuda()
    output1 = Classifier(image)
